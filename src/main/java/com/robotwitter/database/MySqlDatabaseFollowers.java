...
public class MySqlDatabaseFollowers extends AbstractMySqlDatabase implements
	...
	public SqlError deleteFollow(long followedId, long followerId) {
		...
		if (!isExists(followedId, followerId)) {
		...
		try (Connection con = connectionEstablisher.getConnection();
				...
				PreparedStatement preparedStatement = con
						.prepareStatement(String
								.format("UPDATE %s SET %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ? WHERE %s = ?",
	...
	private DBFollower buildFollowerFromResultSet() throws SQLException {
		return new DBFollower(resultSet.getLong(Columns.FOLLOWER_ID.toString()
				.toLowerCase()), resultSet.getString(Columns.NAME.toString()
				...
				resultSet.getBoolean(Columns.IS_CELEBRITY.toString()
						...
						.toString().toLowerCase()),
				resultSet.getString(Columns.PICTURE.toString().toLowerCase()));
	}
