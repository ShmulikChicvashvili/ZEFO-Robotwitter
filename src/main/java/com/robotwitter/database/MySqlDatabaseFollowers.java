		...
		IDatabaseFollowers
{
	...
	public SqlError deleteFollow(long followedId, long followerId)
	{
		...
		if (!isExists(followedId, followerId)) { return SqlError.DOES_NOT_EXIST; }
	...
	public SqlError exportToFile(long userId, String path) throws FileNotFoundException, UnsupportedEncodingException
	{
		...
		ArrayList<DBFollower> followers = getFollowers(userId);
		PrintWriter writer = new PrintWriter(path, "UTF-8");
		writer.write("Id,Name,Screenname,Bio,Number of Followers,Number of Following,Location,Number of Favorites,Language,Celebrity,Joined to Twitter,Picture Link\n");
		for (DBFollower follower: followers) {
			writer.append(follower.toString()+"\n");
		}
		writer.flush();
		writer.close();
		return SqlError.SUCCESS;
