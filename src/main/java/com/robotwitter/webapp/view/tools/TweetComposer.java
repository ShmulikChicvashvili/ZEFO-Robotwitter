...
public class TweetComposer extends RobotwitterCustomComponent
{
	...
	private void initialiseLayout()
	{
		...
		Button submit =
			new Button(
				messages.get("TweetComposer.button.tweet"),
				event -> {
					if (!validateTweet()) { return; }
					clearErrorMessage();
					List<String> tweets =
						tweetingController.breakTweet(tweet.getValue());
					...
					{
						case SUCCESS:
							navigate(ToolsView.NAME);
							Notification notification =
								new Notification(messages
									.get("TweetComposer.notify.success"));
							notification
						...
						default:
							setErrorMessage(messages
								.get("TweetComposer.error.unknown"));
					}
				});
		...
		VerticalLayout left =
			new VerticalLayout(tweet, errorMessage, toolbarAndSubmit);
	...
	final Component createTweetPreview(String tweetText)
	{
		...
		Label text = new Label(tweetText);

		HorizontalLayout nameAndScreenname =
			new HorizontalLayout(nameLabel, screennameLabel);
		...
		nameLabel.addStyleName(PREVIEW_NAME_STYLENAME);
		screennameLabel.addStyleName(PREVIEW_SCREENNAME_STYLENAME);
		text.addStyleName(PREVIEW_TEXT_STYLENAME);
		nameAndScreenname.setStyleName(PREVIEW_NAME_AND_SCREENNAME_STYLENAME);
	...
	private static final String PREVIEW_NAME_AND_SCREENNAME_STYLENAME =
		"TweetComposer-preview-name-and-screenname";
